...
class Verification():
    def __init__(self, config):
        # Inicializa o verificador com as configurações fornecidas
        self.config = config
    ...
    def start(self):
        # Inicia a verificação da estratégia especificada
        print(f'verification strategy: {self.iters}')
        self.bestMetric = self.__getMetric()  # Inicializa a métrica
    ...
    def __getMetric(self):
        ...
        for fileFullPath in fileBlurList:
            ...
            if current_size < batch_size:
                blur = np.pad(blur, ((24, 24), (0, 0), (0, 0)), 'reflect')  # Dividido por 256
                ...
                current_size += 1
            if current_size == batch_size:  # Verifica um lote
    ...
    def __verify(self, epoch):
        # Verifica o desempenho e decide se deve parar
        if epoch % 50 != 0:
            return False
